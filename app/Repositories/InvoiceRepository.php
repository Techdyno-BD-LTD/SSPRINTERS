...
class InvoiceRepository extends BaseRepository
{
    ...
    public function saveInvoice(array $input): Invoice
    {
        try {
            ...
            $invoiceItemInputArray = Arr::only($input, ['product_id', 'quantity','variations', 'price', 'tax', 'tax_id']);
            ...
            $invoiceItemInput[$key]['variations'] = $value['variations'];
            }
            ...
            $input = Arr::only($input, [
                'po_number','style_number','po_value','costing','profit_loss','verdict','client_id', 'invoice_id', 'invoice_date', 'due_date', 'discount_type', 'discount', 'amount', 'final_amount',
            ...
            foreach ($invoiceItemInput as $key => $data) {
                ...
                $data['product_name'] = getproductname($data['product_id']);
    ...
    public function updateInvoice($invoiceId, $input)
    {
        try {
            ...
            foreach ($invoiceItemInput as $key => $value) {
                $total[] = $value['price'] * $value['quantity'];
                $value['variations'] = $value['variations'];
            }
